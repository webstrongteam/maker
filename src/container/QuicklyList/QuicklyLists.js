...
class QuicklyLists extends PureComponent {
    state = {
        quicklyLists: [],
        dropdownData: null,
    ...
    renderDropdownData = () => {
        ...
        dropdownData.push(all, ...categories, newCate);
        this.setState({dropdownData});
    };

    dropdownRenderRow(rowData) {
        const {selectedCategory} = this.state;
        const {tasks, theme} = this.props;
        let data;
        if (rowData.id === -2) {
            data = {
                icon: 'playlist-add',
...
const mapStateToProps = state => {
    return {
        ...
        sorting: state.settings.sortingLists,
...
export default connect(mapStateToProps, mapDispatchToProps)(QuicklyLists);